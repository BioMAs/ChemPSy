...
Med.RMA.Data[,1] <- apply(RMA.Data[,Treated.GZ.CEL.Filenames],1,median)
Med.RMA.Data[,2] <- apply(RMA.Data[,Control.GZ.CEL.Filenames],1,median)
...
if (length(step2) > 0) {
   ...
   Treated.indexes <- which(Design.matrix[,1]==1)
   Ctrl.indexes    <- which(Design.matrix[,2]==1)

   p.values        <- apply(step2.data, 1, function(x) { t.test(x[Treated.indexes], x[Ctrl.indexes]) $p.value } )
   p.BH            <- p.adjust(p.values,method='BH')

   step3      <- step2[which(p.BH <= PV.Cutoff)]
