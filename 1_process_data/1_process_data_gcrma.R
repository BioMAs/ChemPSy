...
#--------------------------------------------#
RLIBS<-"/groups/irset/archives/softs/R/3.1.0/libs"
...
if("gcrma" %in% rownames(installed.packages()) == FALSE) { biocLite("gcrma")}
library(R.utils)
library(affy)
library(gcrma)
library(limma)
...
#---------- CDF env install------------------#
#install.packages("/home/genouest/irset/tdarde/projects/ChemPSy/20171204/data/rat2302rnentrezgprobe_20.0.0.tar.gz", repos = NULL, type="source")
...
# Using GCRMA
Affy.Batch    <- ReadAffy(filenames=sprintf("%s/%s",Output.Dir,CEL.Filenames),sampleNames=GZ.CEL.Filenames)
Affy.Batch@cdfName <- 'rat2302rnentrezg' 
#--------------------------------------------#

#--- RMA normalization ----------------------#
#RMA.Data <- exprs(rma(Affy.Batch))
# Using GCRMA
RMA.Data <- exprs(gcrma(Affy.Batch))
